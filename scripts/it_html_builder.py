...
# ── 증시 한국 연관주 매핑 ─────────────────────────────────────────────────────
# ⚠ 이 딕셔너리는 07_make_stock_report.py의 KR_MAP과 내용이 반드시 같아야 한다.
# 두 파일에 따로 복사돼 있다 보니 한쪽만 갱신하는 사고가 실제로 있었다(^SOX
# 항목 누락). 07_make_stock_report.py가 이 파일을 import하는 구조라, 그쪽은
# 이 딕셔너리를 KR_MAP이라는 이름으로 그대로 재사용하도록 만들어 뒀다 —
# 앞으로 매핑을 바꿀 일이 있으면 반드시 여기 한 곳만 고치면 된다.
STOCK_KR_MAP = {
